 ...
 * Variable to rename espen_ng_ng_oncho_oias_1_site_202207_v2, <%village_dist_required%>
 */
SELECT

  calculate_distance(src.c_gps_lat , src.c_gps_lng , dest.c_gps_lat , dest.c_gps_lng,  'K') "Distances in KM",
  src.c_recorder_id "Code Opérateur 1",
  src.c_site_id "Code Site 1",
  src.c_site_name "Site 1",
  dest.c_recorder_id "Code Opérateur 2",
  dest.c_site_id "Code Site 2",
  dest.c_site_name  "Site 2",
  src.c_gps_lat "Latitude1",
  src.c_gps_lng "Longitude1",
  dest.c_gps_lat "Latitude2",
  dest.c_gps_lng "Longitude2"

FROM espen_ng_ng_oncho_oias_1_site_202207_v2 src
  JOIN espen_ng_ng_oncho_oias_1_site_202207_v2 dest on (
    SELECT calculate_distance(src.c_gps_lat , src.c_gps_lng , dest.c_gps_lat , dest.c_gps_lng,  'K') < 5
    AND src.c_site_id <> dest.c_site_id
  )
