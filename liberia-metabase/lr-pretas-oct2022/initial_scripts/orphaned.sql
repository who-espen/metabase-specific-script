...
/*
 * Variable to rename metabase_lr_lf_tas1_orphaned_202210, v_espen_lr_lf_tas1_2_participant_202210_v0,
...
*/
CREATE TABLE IF NOT EXISTS metabase_lr_lf_tas1_orphaned_202210(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_participant_id_barcode_202210
    ON metabase_lr_lf_tas1_orphaned_202210(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_results_id_barcode_202210
    ON metabase_lr_lf_tas1_orphaned_202210(id_results, barcode_results);

  ALTER TABLE metabase_lr_lf_tas1_orphaned_202210
    ADD CONSTRAINT unique_idx_orphaned_participant_id_barcode_202210
    UNIQUE USING INDEX idx_orphaned_participant_id_barcode_202210;

  ALTER TABLE metabase_lr_lf_tas1_orphaned_202210
    ADD CONSTRAINT unique_idx_orphaned_results_id_barcode_202210
    UNIQUE USING INDEX idx_orphaned_results_id_barcode_202210;
 ...
 */
INSERT INTO metabase_lr_lf_tas1_orphaned_202210(id_participant, recorder_id, barcode_participant, orphaned_type)
...
ON CONFLICT ON CONSTRAINT unique_idx_orphaned_participant_id_barcode_202210 DO NOTHING;
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_orphaned_of_participants p
       RIGHT JOIN metabase_lr_lf_tas1_orphaned_202210 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lr_lf_tas1_orphaned_202210
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_orphaned_of_participants p
            LEFT JOIN metabase_lr_lf_tas1_orphaned_202210 m ON p.id = m.id_participant WHERE p.id IS NOT NULL
 ...
 */
INSERT INTO metabase_lr_lf_tas1_orphaned_202210(id_results, recorder_id, barcode_results, orphaned_type)
  SELECT id, d_recorder_id, d_barcode_id, 'FTS results without participant'
    FROM (
      SELECT
        d.id, d_recorder_id, d_barcode_id
        FROM v_espen_lr_lf_tas1_2_participant_202210_v0 p
         RIGHT JOIN v_espen_lr_lf_tas1_3_resultat_fts_202210 d on p.p_barcode_id = d.d_barcode_id
          WHERE p.id isnull
          ) p
ON CONFLICT ON CONSTRAINT unique_idx_orphaned_results_id_barcode_202210 DO NOTHING;
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_orphaned_of_diag_results p
       RIGHT JOIN public.metabase_lr_lf_tas1_orphaned_202210 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_lr_lf_tas1_orphaned_202210 m ON p.id = m.id_results 
