 ...
 * Variable to rename metabase_gn_lf_pretas_duplicates_202209? espen_ke_lf_pretas_3_resultat_fts_202203_v4
 */
  SELECT
...
FROM (
  SELECT
    ...
    FROM public.metabase_gn_lf_pretas_duplicates_202209 m
    JOIN v_espen_gn_lf_pretas_2_resultat_fts_202210_fts d ON d.id = m.id_results
  ...
  SELECT
   ...
   c_cluster_id cluster_id,
    ...
    FROM public.metabase_gn_lf_pretas_duplicates_202209 m
    JOIN v_espen_gn_lf_pretas_1_site_participant_202210 p ON p.id = m.id_participant::int

  group by p.c_recorder, c_district, c_cluster_id, c_cluster_name
