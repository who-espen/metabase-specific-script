...
*/
CREATE OR REPLACE FUNCTION update_lf_pretas_duplicates_table_from_participant_202108()
 RETURNS void
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_participants p
       RIGHT JOIN metabase_lf_pretas_duplicates_202108 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_pretas_duplicates_202108
          SET status = 'Solved'
          FROM metabase_lf_pretas_duplicates_202108 m
        ...
        --     LEFT JOIN metabase_lf_pretas_duplicates_202108 m ON p.id = m.id_participant
        --     );
...
CREATE OR REPLACE FUNCTION update_lf_pretas_duplicates_table_from_fts_results_202108()
 RETURNS void
-- CREATE OR REPLACE PROCEDURE update_lf_pretas_orphaned_table_from_participant_202108()
LANGUAGE plpgsql
AS $$
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_lf_pretas_duplicates_202108 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
        ...
        --     LEFT JOIN metabase_lf_pretas_duplicates_202108 m ON p.id = m.id_results
        --     );
