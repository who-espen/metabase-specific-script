...
CREATE TABLE IF NOT EXISTS metabase_lf_tas1_duplicates_202105(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_participant_id_barcode_202105
    ON metabase_lf_tas1_duplicates_202105(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_id_barcode_202105
    ON metabase_lf_tas1_duplicates_202105(id_results, barcode_results);

  ALTER TABLE metabase_lf_tas1_duplicates_202105
...
SELECT src.id, src.p_generate_id FROM v_espen_sn_lf_tas1_2_partcipants_202105 src
                WHERE (SELECT count (*)  FROM v_espen_sn_lf_tas1_2_partcipants_202105 inr WHERE src.p_generate_id = inr.p_generate_id ) > 1;
...
SELECT src.id, src.d_generate_id FROM v_espen_sn_lf_tas1_3_fts_result_202105 src
                WHERE (SELECT count (*)  FROM v_espen_sn_lf_tas1_3_fts_result_202105 inr WHERE src.d_generate_id = inr.d_generate_id ) > 1;
