...
CREATE TABLE IF NOT EXISTS metabase_ci_lf_pretas_202302_duplicates(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_participant_id_barcode_202302_pretas
    ON metabase_ci_lf_pretas_202302_duplicates(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_id_barcode_202302_pretas
    ON metabase_ci_lf_pretas_202302_duplicates(id_results, barcode_results);

  ALTER TABLE metabase_ci_lf_pretas_202302_duplicates
...
SELECT src.id, src.p_code_id FROM v_espen_ci_lf_pretas_2_participant_202302 src
                WHERE (SELECT count (*)  FROM v_espen_ci_lf_pretas_2_participant_202302 inr WHERE src.p_code_id = inr.p_code_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_participants_202302_pretas p
       RIGHT JOIN metabase_ci_lf_pretas_202302_duplicates m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_ci_lf_pretas_202302_duplicates
          SET status = 'Solved'
          FROM metabase_ci_lf_pretas_202302_duplicates m
        ...
        --     LEFT JOIN metabase_ci_lf_pretas_202302_duplicates m ON p.id = m.id_participant
        --     );

      END IF;

       INSERT INTO metabase_ci_lf_pretas_202302_duplicates(id_participant, barcode_participant, form)
...
SELECT src.id, src.d_code_id FROM v_espen_ci_lf_pretas_3_resultat_fts_202302 src
                WHERE (SELECT count (*)  FROM v_espen_ci_lf_pretas_3_resultat_fts_202302 inr WHERE src.d_code_id = inr.d_code_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_ci_lf_pretas_202302_duplicates m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
        ...
        --     LEFT JOIN metabase_ci_lf_pretas_202302_duplicates m ON p.id = m.id_results
        --     );

      END IF;

       INSERT INTO metabase_ci_lf_pretas_202302_duplicates(id_results, barcode_results, form)
