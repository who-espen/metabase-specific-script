 ...
 * Variable to rename <%Epirf_Survey_Type%>, v_espen_ci_lf_pretas_3_resultat_fts_202302, v_espen_ci_lf_pretas_2_participant_202302,
 ...
 */
  ...
  count(p.id) "Examiné",
  COUNT(CASE WHEN d_result = 'Positif' THEN 1 ELSE NULL END) "Number de Positif",
  ROUND(COUNT(CASE WHEN d_result = 'Positif' THEN 1 ELSE NULL END) * 100.0 / count(p.id), 2) "% positif",
...
LEFT JOIN v_espen_ci_lf_pretas_1_site_202302 c on d.d_site_id = c.c_site_id
