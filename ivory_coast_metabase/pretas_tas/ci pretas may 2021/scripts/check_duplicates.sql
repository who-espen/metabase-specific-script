...
*/
CREATE OR REPLACE FUNCTION update_lf_retas1_duplicates_table_from_participant_202105_2()
 RETURNS void
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_tas_duplicates_of_participants p
       RIGHT JOIN metabase_bj_lf_retas1_duplicates_202105_2 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_bj_lf_retas1_duplicates_202105_2
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_tas_duplicates_of_participants p
            LEFT JOIN metabase_bj_lf_retas1_duplicates_202105_2 m ON p.id = m.id_participant
            WHERE p.id IS NOT NULL
            ) and form = 'Participant' and survey_designation = 'TAS2';

      END IF;

       INSERT INTO metabase_bj_lf_retas1_duplicates_202105_2(id_participant, barcode_participant, form, survey_designation)
...
CREATE OR REPLACE FUNCTION update_lf_retas1_duplicates_table_from_fts_results_202105_2()
 RETURNS void
-- CREATE OR REPLACE PROCEDURE update_lf_retas1_orphaned_table_from_participant()
LANGUAGE plpgsql
AS $$
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_bj_lf_retas1_duplicates_202105_2 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_bj_lf_retas1_duplicates_202105_2 m ON p.id = m.id_results            
            WHERE p.id IS NOT NULL
            ) and form = 'Résultat FTS' and survey_designation = 'TAS2';

      END IF;

       INSERT INTO metabase_bj_lf_retas1_duplicates_202105_2(id_results, barcode_results, form, survey_designation)
