...
SELECT
    ...
    COUNT(
        CASE
            WHEN (resultat_fts1 = 'Positive') THEN 1
            ELSE NULL
        END
    ) "Total Positive",
    COUNT(
    ...
    COUNT(
        CASE
            WHEN (resultat_fts1 = 'Negative') THEN 1
            ELSE NULL
        END
    ) "Total Negative",
    COUNT(
        CASE
            WHEN (resultat_fts1 = 'Invalid') THEN 1
            ELSE NULL
        END
    ) "Total Invalid"
FROM
    v_espen_bf_lf_tas1_202306_3_resultats_fts_v4 d
    LEFT JOIN espen_bf_lf_tas3_1_sites_202307 c on d.d_cluster_id:: int = c.nb_grappe
    RIGHT JOIN v_espen_bf_lf_tas1_202306_2_participant_v4 p on p.barcode = d.barcode
WHERE
