    ...
    FROM (
      SELECT
        ...
        FROM v_espen_bf_lf_tas1_2_enrolement_202010 p
         LEFT JOIN v_espen_bf_lf_tas1_3_resultat_fts_202010 d on p.barcode = d.barcode
          WHERE d.id isnull
          ) p
ON CONFLICT ON CONSTRAINT unique_idx_orphaned_participant_id_barcode_tas_202011 DO NOTHING;
  ...
  SELECT id, code_operateur, barcode, 'FTS results without participant'
    FROM (
      SELECT
        ...
        FROM v_espen_bf_lf_tas1_2_enrolement_202010 p
         RIGHT JOIN v_espen_bf_lf_tas1_3_resultat_fts_202010 d on p.barcode = d.barcode
          WHERE p.id isnull
          ) p
ON CONFLICT ON CONSTRAINT unique_idx_orphaned_results_id_barcode_tas_202011 DO NOTHING;
