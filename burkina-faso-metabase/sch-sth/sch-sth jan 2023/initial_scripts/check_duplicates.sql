...
LANGUAGE PLPGSQL AS $$ 
	BEGIN
	    ...
	    CREATE
	    OR REPLACE TEMPORARY VIEW v_bf_202301_sch_sth_duplicates_of_participants AS
	SELECT
	    src.id,
	    src.p_code_site
	FROM
	    v_espen_bf_202301_sch_impact_assessment_2_enrolement_v2 src
	WHERE (
	        SELECT count (*)
	        FROM
	            v_espen_bf_202301_sch_impact_assessment_2_enrolement_v2 inr
	        WHERE
...
LANGUAGE PLPGSQL AS $$ 
	...
	SELECT
	    ...
	    src.k_code_site
	FROM
	    v_espen_bf_202301_sch_impact_assessment_3_resultat_selles_v3 src
	WHERE (
	        SELECT count (*)
	        FROM
	            v_espen_bf_202301_sch_impact_assessment_3_resultat_selles_v3 inr
	        WHERE
...
LANGUAGE PLPGSQL AS $$ 
	...
	SELECT
	    ...
	    src.u_barcodeid
	FROM
	    v_espen_bf_202301_sch_impact_assessment_4_resultat_urines_v3 src
	WHERE (
	        SELECT count (*)
	        FROM
	            v_espen_bf_202301_sch_impact_assessment_4_resultat_urines_v3 inr
	        WHERE
