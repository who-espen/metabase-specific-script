...
-- Create a view to get the list of duplicates participants --.
 CREATE OR REPLACE TEMPORARY VIEW v_lf_duplicates_of_participants_202104 AS
SELECT src.id, src.p_generate_id FROM v_espen_sn_lf_tas1_2_partcipants_202104_v2 src
                WHERE (SELECT count (*)  FROM v_espen_sn_lf_tas1_2_partcipants_202104_v2 inr WHERE src.p_generate_id = inr.p_generate_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_participants_202104 p
       RIGHT JOIN metabase_lf_tas1_duplicates_202104 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_tas1_duplicates_202104
          SET status = 'Solved'
          FROM metabase_lf_tas1_duplicates_202104 m
      ...
      END IF;
       ...
       INSERT INTO metabase_lf_tas1_duplicates_202104(id_participant, barcode_participant, form)
        SELECT id, p_generate_id, 'Participant'
            FROM (
              SELECT src.id, src.p_generate_id FROM v_espen_sn_lf_tas1_2_partcipants_202104_v2 src
                WHERE (SELECT count (*)  FROM v_espen_sn_lf_tas1_2_partcipants_202104_v2 inr WHERE src.p_generate_id = inr.p_generate_id ) > 1
            ) p
        ON CONFLICT ON CONSTRAINT unique_idx_duplicates_participant_id_barcode_202104 DO NOTHING;
...
SELECT src.id, src.d_generate_id FROM v_espen_sn_lf_tas1_3_fts_result_202104_v2 src
                WHERE (SELECT count (*)  FROM v_espen_sn_lf_tas1_3_fts_result_202104_v2 inr WHERE src.d_generate_id = inr.d_generate_id ) > 1;
...
-- Check if there is solved orphaned participant then update
      ...
      END IF;
       ...
       INSERT INTO metabase_lf_tas1_duplicates_202104(id_results, barcode_results, form)
        SELECT id, d_generate_id, 'Résultat FTS'
            FROM (
              SELECT src.id, src.d_generate_id FROM v_espen_sn_lf_tas1_3_fts_result_202104_v2 src
                WHERE (SELECT count (*)  FROM v_espen_sn_lf_tas1_3_fts_result_202104_v2 inr WHERE src.d_generate_id = inr.d_generate_id ) > 1
            ) p
        ON CONFLICT ON CONSTRAINT unique_idx_duplicates_results_id_barcode_202104 DO NOTHING;
