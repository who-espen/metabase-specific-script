...
/*
 * Variable to rename metabase_lf_pretas_orphaned_202009, v_espen_sn_lf_pretas_2_partcipants_202009_manual,
...
*/
CREATE TABLE IF NOT EXISTS metabase_lf_pretas_orphaned_202009(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_participant_id_barcode
    ON metabase_lf_pretas_orphaned_202009(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_results_id_barcode
    ON metabase_lf_pretas_orphaned_202009(id_results, barcode_results);

  ALTER TABLE metabase_lf_pretas_orphaned_202009
 ...
 */
INSERT INTO metabase_lf_pretas_orphaned_202009(id_participant, recorder_id, barcode_participant, orphaned_type)
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_orphaned_of_participants p
       RIGHT JOIN metabase_lf_pretas_orphaned_202009 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_pretas_orphaned_202009
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_orphaned_of_participants p
            LEFT JOIN metabase_lf_pretas_orphaned_202009 m ON p.id = m.id_participant WHERE p.id IS NOT NULL
 ...
 */
INSERT INTO metabase_lf_pretas_orphaned_202009(id_results, recorder_id, barcode_results, orphaned_type)
  SELECT id, d_recorder_id, d_barcode_id, 'FTS results without participant'
    FROM (
      SELECT
        d.id, d_recorder_id, d_barcode_id
        FROM v_espen_sn_lf_pretas_2_partcipants_202009_manual p
         RIGHT JOIN v_espen_bj_lf_pretas_3_fts_result_202009 d on p.p_barcode_id = d.d_barcode_id
          WHERE p.id isnull
          ) p
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_orphaned_of_diag_results p
       RIGHT JOIN public.metabase_lf_pretas_orphaned_202009 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_lf_pretas_orphaned_202009 m ON p.id = m.id_results 
