...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_participants p
       RIGHT JOIN metabase_lf_pretas_duplicates_202009 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_pretas_duplicates_202009
          SET status = 'Solved'
          FROM metabase_lf_pretas_duplicates_202009 m
        ...
        --     LEFT JOIN metabase_lf_pretas_duplicates_202009 m ON p.id = m.id_participant
        --     );
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_lf_pretas_duplicates_202009 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
        ...
        --     LEFT JOIN metabase_lf_pretas_duplicates_202009 m ON p.id = m.id_results
        --     );
