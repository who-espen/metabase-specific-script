 ...
 * Variable to rename espen_cm_oncho_pre_stop_1_202309_village_v2, <%village_dist_required%>
 */
SELECT
...
--  src.c_village_id "Cluster1 ID",
  src.c_village "Cluster1 Name",
  dest.c_recorder_id "Recorder2 ID",
 -- dest.c_village_id "Cluster2 ID",
  dest.c_village  "Cluster2 Name",
  src.c_gps_lat "Latitude1",
  src.c_gps_lng "Longitude1",
  dest.c_gps_lat "Latitude2",
  dest.c_gps_lng "Longitude2"

FROM public.v_espen_cm_oncho_pre_stop_1_202309_village_v2 src
  JOIN public.v_espen_cm_oncho_pre_stop_1_202309_village_v2 dest on (
    SELECT calculate_distance(src.c_gps_lat , src.c_gps_lng , dest.c_gps_lat , dest.c_gps_lng,  'K') < 5
    AND src.c_village <> dest.c_village
  )
