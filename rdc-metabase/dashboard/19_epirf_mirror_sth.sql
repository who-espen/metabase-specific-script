...
SELECT
 ...
 'Kato-Katz' "Test Diagnostique",
  count(case when k_ascaris_lumb_intensity = 0 or k_ascaris_lumb_intensity > 0 then 1 else null end) "Ascaris - Nbr examiné",
  count(case when k_ascaris_lumb_intensity > 0 then 1 else null end) "Ascaris - Nbr Positif",
  count(case when k_ascaris_lumb_intensity > 0 then 1 else null end)*100/count(*) "Ascaris - % Positif",
  ...
  count(case when k_hookworm_intensity > 0 then 1 else null end)*100/count(*) "Ankylostome - % Positif",
  ...
  count(case when k_trichuris_intensity > 0 then 1 else null end)*100/count(*) "Trichuris - % Positif",
  ...
  count(case when (k_ascaris_lumb_intensity > 0 or k_hookworm_intensity > 0 or k_trichuris_intensity > 0) then 1 else null end)*100/count(*) "Géohelminthiases - % Positif",

  INITCAP(p_subdistrict) "USP"
  
FROM public.v_espen_cd_sch_sth_baseline_2_child_202202_v2_1 p

left join public.v_espen_cd_sch_sth_baseline_1_school_202202_v2_1 w on w.w_site_name = p.p_site
left join public.v_espen_cd_sch_sth_baseline_3_kato_katz_202202_v2_1 k on k.k_generate_id = p.p_generate_id
left join public.v_espen_cd_sch_sth_baseline_4_urinefiltration_202202_v2_1 u on u.u_generate_id = p.p_generate_id

where k_generate_id is not null



GROUP BY p_district, p_site, p_subdistrict ,w_start , w_gps_lat, w_gps_lng
