...
/*
 * Variable to rename metabase_sch_sth_mapping_duplicates_202107, <%identify_participant_duplicate%>, v_espen_bw_sch_sth_mid_term_evaluation_2_participant_v2_1
 * v_espen_bw_sch_sth_mid_term_evaluation_2_participant_v2_1, <%metabase_sch_sth_ias_duplicates_202005_trigger%>, <%identify_kk_result_duplicate%>,
 * v_espen_bw_sch_sth_mid_term_evaluation_3_kato_katz_v2, v_espen_bw_sch_sth_mid_term_evaluation_3_kato_katz_v2, <%metabase_sch_sth_kk_result_duplicates_202004_trigger%>, <%identify_urine_result_duplicate%>
 * v_espen_bw_sch_sth_mid_term_evaluation_4_urinefiltration_cca, v_espen_bw_sch_sth_mid_term_evaluation_4_urinefiltration_cca
 */
...
*/
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_kk_id_barcode
    ON metabase_sch_sth_mapping_duplicates_202107(id_results_kk, barcode_results);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_urine_id_barcode
    ON metabase_sch_sth_mapping_duplicates_202107(id_results_urine, barcode_results);
   ...
   BEGIN
      ...
      IF EXISTS(
        SELECT src.id, src.p_generateid FROM v_espen_bw_sch_sth_mid_term_evaluation_2_participant_v2_1 src
          WHERE src.p_generateid = NEW.p_generateid
            AND (SELECT count (*)  FROM v_espen_bw_sch_sth_mid_term_evaluation_2_participant_v2_1 inr WHERE src.p_generateid = inr.p_generateid ) > 1
            ) THEN
        ...
        INSERT INTO metabase_sch_sth_mapping_duplicates_202107(id_participant, barcode_participant, form)
          SELECT id, p_generateid, 'Participant'
            FROM (SELECT src.id, src.p_generateid FROM v_espen_bw_sch_sth_mid_term_evaluation_2_participant_v2_1 src
              WHERE src.p_generateid = NEW.p_generateid) p
...
CREATE TRIGGER <%metabase_sch_sth_ias_duplicates_202005_trigger%> AFTER INSERT OR UPDATE OR DELETE ON v_espen_bw_sch_sth_mid_term_evaluation_2_participant_v2_1
...
CREATE OR REPLACE FUNCTION <%identify_kk_result_duplicate%>() RETURNS TRIGGER AS $$
   BEGIN
      ...
      IF EXISTS(
        SELECT src.id, k_generate_id FROM v_espen_bw_sch_sth_mid_term_evaluation_3_kato_katz_v2 src
          WHERE k_generate_id = NEW.k_generate_id
            AND (SELECT count (*)  FROM v_espen_bw_sch_sth_mid_term_evaluation_3_kato_katz_v2 inr WHERE src.k_generate_id = inr.k_generate_id ) > 1
            ) THEN

        INSERT INTO metabase_sch_sth_mapping_duplicates_202107(id_results_kk, barcode_results, form)
          SELECT id, k_generate_id, 'Kato Katz Results'
            FROM (SELECT src.id, k_generate_id FROM v_espen_bw_sch_sth_mid_term_evaluation_3_kato_katz_v2 src
...
CREATE TRIGGER <%metabase_sch_sth_kk_result_duplicates_202004_trigger%> AFTER INSERT OR UPDATE OR DELETE ON v_espen_bw_sch_sth_mid_term_evaluation_3_kato_katz_v2
...
CREATE OR REPLACE FUNCTION <%identify_urine_result_duplicate%>() RETURNS TRIGGER AS $$
   BEGIN
      ...
      IF EXISTS(
        SELECT src.id, u_generate_id FROM v_espen_bw_sch_sth_mid_term_evaluation_4_urinefiltration_cca src
          WHERE u_generate_id = NEW.u_generate_id
            AND (SELECT count (*)  FROM v_espen_bw_sch_sth_mid_term_evaluation_4_urinefiltration_cca inr WHERE src.u_generate_id = inr.u_generate_id ) > 1
            ) THEN

        INSERT INTO metabase_sch_sth_mapping_duplicates_202107(id_results_urine, barcode_results, form)
          SELECT id, u_generate_id, 'Urine Results'
            FROM (SELECT src.id, u_generate_id FROM v_espen_bw_sch_sth_mid_term_evaluation_4_urinefiltration_cca src
...
CREATE TRIGGER <%metabase_sch_sth_urine_result_duplicates_202004_trigger%> AFTER INSERT OR UPDATE OR DELETE ON v_espen_bw_sch_sth_mid_term_evaluation_4_urinefiltration_cca
