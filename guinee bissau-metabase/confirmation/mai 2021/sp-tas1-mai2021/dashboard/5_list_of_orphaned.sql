 ...
 * Variable to rename v_espen_gw_lf_conf_2_partcipants_202206_v2
 ...
 */
 ...
 d on p.p_barcode_id = d.d_barcode_id
      LEFT JOIN metabase_lf_conf_orphaned_202206 m on m.id_participant = p.id
      WHERE d.id isNULL
  ...
  -- [[and {{date}}]]
    ...
    FROM v_espen_gw_lf_conf_3_fts_result_202206_v2
 d
      LEFT JOIN v_espen_gw_lf_conf_2_partcipants_202206_v2
 p on p.p_barcode_id = d.d_barcode_id
      LEFT JOIN metabase_lf_conf_orphaned_202206 m on m.id_results = d.id
      WHERE p.id isNULL
