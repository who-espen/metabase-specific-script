 ...
 * Variable to rename Pre TAS, v_espen_mz_lf_tas2_3_resultat_fts_202101, espen_mz_lf_tas2_2_participant_202101,
 * update_lf_tas_orphaned_table_from_diag_result_202101
 */
 SELECT
  ...
  TO_CHAR(c.c_end, 'Month') "Month",
  EXTRACT(YEAR FROM c.c_end) "Year",
  c_gps_lat "Latitude",
  c_gps_lng "Longitude",
...
--  end  "Number of PC Round",
  ...
  c_ending_survey_note::int "Target Sample Size",-- TODO: Update the sample size
  count(p.id) "Examinded",
  COUNT(CASE WHEN d_final_result = 'Positivo' THEN 1 ELSE NULL END) "Number of Positive",
  ROUND(COUNT(CASE WHEN d_final_result = 'Positivo' THEN 1 ELSE NULL END) * 100.0 / c_ending_survey_note::int, 2) "% positive",
...
JOIN v_espen_mz_lf_tas2_1_site_202101 c on d.d_cluster_id = c.c_cluster_id
JOIN v_espen_mz_lf_tas2_2_participant_202101 p on p.p_code_id = d.d_code_id

GROUP BY c_eu, c_communr, c_cluster_name, "Month", "Year", c_gps_lat, c_gps_lng, c_ending_survey_note
