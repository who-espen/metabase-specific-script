 ...
 * Variable to rename metabase_lf_tas2_duplicates_202112? espen_mz_lf_tas2_3_resultat_fts_202111_v2
 */
 SELECT
  "Gravadors",
...
FROM (
  SELECT
    ...
    FROM public.metabase_lf_tas2_duplicates_202112 m
    JOIN v_espen_mz_lf_tas2_3_resultat_fts_202111_v2 d ON d.id = m.id_results
    
    --where survey_designation ='TAS2'

    GROUP BY d.d_recorder_id, d_eu, d_cluster_id, d_cluster_name, d.id
  ...
  SELECT
    ...
    FROM public.metabase_lf_tas2_duplicates_202112 m
    JOIN v_espen_mz_lf_tas1_2_participant_202011_v3 p ON p.id = m.id_participant
    
     --where survey_designation ='TAS2'

  group by p.p_recorder_id, p_eu, p_cluster_id, p_cluster_name, p.id
