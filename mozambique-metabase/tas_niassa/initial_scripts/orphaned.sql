...
/*
 * Variable to rename metabase_lf_tas_orphaned_202012, v_espen_mz_lf_tas1_2_participant_202011_v3,
...
*/
CREATE TABLE IF NOT EXISTS metabase_lf_tas_orphaned_202012(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_participant_id_barcode
    ON metabase_lf_tas_orphaned_202012(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_results_id_barcode
    ON metabase_lf_tas_orphaned_202012(id_results, barcode_results);

  ALTER TABLE metabase_lf_tas_orphaned_202012
 ...
 */
INSERT INTO metabase_lf_tas_orphaned_202012(id_participant, recorder_id, barcode_participant, orphaned_type)
 ...
 */
CREATE OR REPLACE FUNCTION public.update_lf_tas_orphaned_table_from_participant()
 RETURNS void
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_orphaned_of_participants p
       RIGHT JOIN metabase_lf_tas_orphaned_202012 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_tas_orphaned_202012
          SET status = 'Solved'          
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_orphaned_of_participants p
            LEFT JOIN metabase_lf_tas_orphaned_202012 m ON p.id = m.id_participant
 ...
 */
INSERT INTO metabase_lf_tas_orphaned_202012(id_results, recorder_id, barcode_results, orphaned_type)
  SELECT id, d_recorder_id, d_code_id, 'FTS results without participant'
    FROM (
      SELECT
        d.id, d_recorder_id, d_code_id
        FROM v_espen_mz_lf_tas1_2_participant_202011_v3 p
         RIGHT JOIN v_espen_mz_lf_tas1_3_resultat_fts_202011_v3 d on p.p_code_id = d.d_code_id
          WHERE p.id isnull
          ) p
 ...
 */
 CREATE OR REPLACE FUNCTION public.update_lf_tas_orphaned_table_from_diag_result()
 RETURNS void
-- CREATE OR REPLACE PROCEDURE update_lf_tas_orphaned_table_from_diag_result()
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_orphaned_of_diag_results p
       RIGHT JOIN public.metabase_lf_tas_orphaned_202012 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_lf_tas_orphaned_202012 m ON p.id = m.id_results            
             WHERE p.id IS NOT NULL
