...
/*
 * Variable to rename metabase_lf_tas_duplicates_202012, identify_participant_duplicate_tas_202012, v_espen_mz_lf_tas1_2_participant_202011_v3,
 * v_espen_mz_lf_tas1_3_resultat_fts_202011_v3, metabase_lf_tas_result_duplicates_202012_trigger, metabase_lf_tas_duplicates_202012_trigger,
...
*/
CREATE TABLE IF NOT EXISTS metabase_lf_tas_duplicates_202012(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_participant_id_barcode
    ON metabase_lf_tas_duplicates_202012(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_id_barcode
    ON metabase_lf_tas_duplicates_202012(id_results, barcode_results);

  ALTER TABLE metabase_lf_tas_duplicates_202012
...
* Returns: trigger
*/
CREATE OR REPLACE FUNCTION identify_participant_duplicate_tas_202012() RETURNS TRIGGER AS $$
   BEGIN

      IF EXISTS(
        SELECT src.id, src.p_code_id FROM v_espen_mz_lf_tas1_2_participant_202011_v3 src
          WHERE src.p_code_id = NEW.p_code_id
            AND (SELECT count (*)  FROM v_espen_mz_lf_tas1_2_participant_202011_v3 inr WHERE src.p_code_id = inr.p_code_id ) > 1
            ) THEN

        INSERT INTO metabase_lf_tas_duplicates_202012(id_participant, barcode_participant, form)
          SELECT id, p_code_id, 'Participant'
            FROM (SELECT src.id, src.p_code_id FROM v_espen_mz_lf_tas1_2_participant_202011_v3 src
...
-- CREATE TRIGGER metabase_lf_tas_duplicates_202012_trigger AFTER INSERT OR UPDATE OR DELETE ON espen_mz_lf_tas1_2_participant_202011_v3
-- FOR EACH ROW EXECUTE PROCEDURE identify_participant_duplicate_tas_202012();
...
* Returns: trigger
*/
CREATE OR REPLACE FUNCTION identify_tas_diag_result_duplicate_202012()
...
AS $$
   BEGIN
      ...
      IF EXISTS(
        SELECT src.id, d_code_id FROM v_espen_mz_lf_tas1_3_resultat_fts_202011_v3 src
          WHERE d_code_id = NEW.d_code_id
            AND (SELECT count (*)  FROM v_espen_mz_lf_tas1_3_resultat_fts_202011_v3 inr WHERE src.d_code_id = inr.d_code_id ) > 1
            ) THEN

        INSERT INTO metabase_lf_tas_duplicates_202012(id_results, barcode_results, form)
          SELECT id, d_code_id, 'Diagnostic'
            FROM (SELECT src.id, d_code_id FROM v_espen_mz_lf_tas1_3_resultat_fts_202011_v3 src
...
CREATE TRIGGER metabase_lf_tas_result_duplicates_202012_trigger AFTER INSERT OR UPDATE OR DELETE ON espen_mz_lf_tas1_3_resultat_fts_202011_v3
FOR EACH ROW EXECUTE PROCEDURE identify_tas_diag_result_duplicate_202012();
