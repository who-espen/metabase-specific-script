 ...
 * Variable to rename v_espen_bi_oncho_prestop_2_202307_questions_v3
 */
...
FROM (
        SELECT
            COUNT(
                case
                    when p_sex = 'Homme' then 1
                    else null
                end
            ) male
        FROM
            public.v_espen_bi_oncho_prestop_2_202307_questions_v3 P
        GROUP BY
            p_cluster_name,
            c_start
    ) src
