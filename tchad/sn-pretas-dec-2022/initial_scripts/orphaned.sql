...
/*
 * Variable to rename metabase_td_lf_pretas_orphaned_202212, v_espen_td_lf_pretas_2_participant_202208_v2,
...
*/
CREATE TABLE IF NOT EXISTS metabase_td_lf_pretas_orphaned_202212(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_participant_id_barcode_202212
    ON metabase_td_lf_pretas_orphaned_202212(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_results_id_barcode_202212
    ON metabase_td_lf_pretas_orphaned_202212(id_results, barcode_results);

  ALTER TABLE metabase_td_lf_pretas_orphaned_202212
 ...
 */
INSERT INTO metabase_td_lf_pretas_orphaned_202212(id_participant, recorder_id, barcode_participant, orphaned_type)
    ...
    FROM (
      SELECT
        ...
        FROM v_espen_td_lf_pretas_2_participant_202208_v2 p
         LEFT JOIN v_espen_td_lf_pretas_3_resultat_fts_202208_v2 d on p.p_barcode_id = d.d_barcode_id
          WHERE d.id isnull
          ) p
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_orphaned_of_participants p
       RIGHT JOIN metabase_td_lf_pretas_orphaned_202212 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_td_lf_pretas_orphaned_202212
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_orphaned_of_participants p
            LEFT JOIN metabase_td_lf_pretas_orphaned_202212 m ON p.id = m.id_participant WHERE p.id IS NOT NULL
 ...
 */
INSERT INTO metabase_td_lf_pretas_orphaned_202212(id_results, recorder_id, barcode_results, orphaned_type)
  SELECT id, d_recorder_id, d_barcode_id, 'FTS results without participant'
    FROM (
      SELECT
        d.id, d_recorder_id, d_barcode_id
        FROM v_espen_td_lf_pretas_2_participant_202208_v2 p
         RIGHT JOIN v_espen_td_lf_pretas_3_resultat_fts_202208_v2 d on p.p_barcode_id = d.d_barcode_id
          WHERE p.id isnull
          ) p
 ...
 */
 ...
 RETURNS void
-- CREATE OR REPLACE PROCEDURE update_lf_pretas_orphaned_table_from_diag_result_202212()
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_orphaned_of_diag_results p
       RIGHT JOIN public.metabase_td_lf_pretas_orphaned_202212 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_td_lf_pretas_orphaned_202212 m ON p.id = m.id_results 
