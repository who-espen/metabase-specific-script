...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_participants p
       RIGHT JOIN metabase_ng_lf_tas_duplicates_202405 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_ng_lf_tas_duplicates_202405
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_duplicates_of_participants p
            LEFT JOIN metabase_ng_lf_tas_duplicates_202405 m ON p.id = m.id_participant
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_ng_lf_tas_duplicates_202405 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_ng_lf_tas_duplicates_202405 m ON p.id = m.id_results            
            WHERE p.id IS NOT NULL
