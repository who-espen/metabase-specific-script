CREATE TABLE IF NOT EXISTS metabase_ng_lf_2410_tas1_duplicates_kg(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_participant_id_barcode_lf_tas_202405
    ON metabase_ng_lf_2410_tas1_duplicates_kg(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_id_barcode_lf_tas_202405
    ON metabase_ng_lf_2410_tas1_duplicates_kg(id_results, barcode_results);
...
SELECT src.id, src.p_code_id FROM v_espen_ng_lf_tas_2410_1_sit_part_kg src
                WHERE (SELECT count (*)  FROM v_espen_ng_lf_tas_2410_1_sit_part_kg inr WHERE src.p_code_id = inr.p_code_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_participants p
       RIGHT JOIN metabase_ng_lf_2410_tas1_duplicates_kg m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_ng_lf_2410_tas1_duplicates_kg
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_duplicates_of_participants p
            LEFT JOIN metabase_ng_lf_2410_tas1_duplicates_kg m ON p.id = m.id_participant
            WHERE p.id IS NOT NULL
            ) and form = 'Participant';

      END IF;

       INSERT INTO metabase_ng_lf_2410_tas1_duplicates_kg(id_participant, barcode_participant, form)
...
SELECT src.id, src.d_code_id FROM v_espen_ng_lf_tas_2410_2_fts_kg src
                WHERE (SELECT count (*)  FROM v_espen_ng_lf_tas_2410_2_fts_kg inr WHERE src.d_code_id = inr.d_code_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_ng_lf_2410_tas1_duplicates_kg m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_ng_lf_2410_tas1_duplicates_kg m ON p.id = m.id_results            
            WHERE p.id IS NOT NULL
            ) and form = 'Résultat FTS';

      END IF;

       INSERT INTO metabase_ng_lf_2410_tas1_duplicates_kg(id_results, barcode_results, form)
