 ...
 select
    ...
    where (select count(*) from v_espen_ng_lf_tas_2410_1_sit_part_kg k where p.p_code_id = k.p_code_id) > 1
    ...
    FROM public.v_espen_ng_lf_tas_2410_2_fts_kg d
    where (select count(*) from v_espen_ng_lf_tas_2410_2_fts_kg r where d.d_code_id = r.d_code_id) > 1