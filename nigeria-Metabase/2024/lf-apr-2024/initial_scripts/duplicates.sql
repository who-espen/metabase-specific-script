...
/*
 * Variable to rename metabase_ng_lf_pretas_duplicates_202404, identify_participant_duplicate_pretas_202212, v_espen_ng_lf_pretas_202404_2_participant_v2,
 * v_espen_ng_lf_pretas_202404_3_fts_v2, metabase_ng_lf_pretas_result_duplicates_202212_trigger, metabase_ng_lf_pretas_duplicates_202404_trigger,
      ...
      IF EXISTS(
        SELECT src.id, src.p_code_id FROM v_espen_ng_lf_pretas_202404_2_participant_v2 src
          WHERE src.p_code_id = NEW.p_code_id
            AND (SELECT count (*)  FROM v_espen_ng_lf_pretas_202404_2_participant_v2 inr WHERE src.p_code_id = inr.p_code_id ) > 1
            ) THEN
        ...
        INSERT INTO metabase_ng_lf_pretas_duplicates_202404(id_participant, barcode_participant, form)
          SELECT id, p_code_id, 'Participant'
            FROM (SELECT src.id, src.p_code_id FROM v_espen_ng_lf_pretas_202404_2_participant_v2 src
...
-- CREATE TRIGGER metabase_ng_lf_pretas_duplicates_202404_trigger AFTER INSERT OR UPDATE OR DELETE ON espen_ng_lf_pretas_202404_2_participant_v2
...
AS $$
   BEGIN
      ...
      IF EXISTS(
        SELECT src.id, d_code_id FROM v_espen_ng_lf_pretas_202404_3_fts_v2 src
          WHERE d_code_id = NEW.d_code_id
            AND (SELECT count (*)  FROM v_espen_ng_lf_pretas_202404_3_fts_v2 inr WHERE src.d_code_id = inr.d_code_id ) > 1
            ) THEN

        INSERT INTO metabase_ng_lf_pretas_duplicates_202404(id_results, barcode_results, form)
          SELECT id, d_code_id, 'Diagnostic'
            FROM (SELECT src.id, d_code_id FROM v_espen_ng_lf_pretas_202404_3_fts_v2 src
...
CREATE TRIGGER metabase_ng_lf_pretas_result_duplicates_202212_trigger AFTER INSERT OR UPDATE OR DELETE ON espen_ng_lf_tas_202404_3_resultat_fts_jgw_v3
