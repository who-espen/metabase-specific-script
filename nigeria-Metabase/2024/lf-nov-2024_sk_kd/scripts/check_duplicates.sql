CREATE TABLE IF NOT EXISTS metabase_ng_lf_2411_tas1_duplicates_sk_kd(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_participant_id_barcode_lf_tas_202405
    ON metabase_ng_lf_2411_tas1_duplicates_sk_kd(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_id_barcode_lf_tas_202405
    ON metabase_ng_lf_2411_tas1_duplicates_sk_kd(id_results, barcode_results);
...
SELECT src.id, src.p_code_id FROM v_espen_ng_lf_tas_2411_2_fts_yb_v2_3_v2 src
                WHERE (SELECT count (*)  FROM v_espen_ng_lf_tas_2411_2_fts_yb_v2_3_v2 inr WHERE src.p_code_id = inr.p_code_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_participants p
       RIGHT JOIN metabase_ng_lf_2411_tas1_duplicates_sk_kd m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_ng_lf_2411_tas1_duplicates_sk_kd
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_duplicates_of_participants p
            LEFT JOIN metabase_ng_lf_2411_tas1_duplicates_sk_kd m ON p.id = m.id_participant
            WHERE p.id IS NOT NULL
            ) and form = 'Participant';

      END IF;

       INSERT INTO metabase_ng_lf_2411_tas1_duplicates_sk_kd(id_participant, barcode_participant, form)
...
SELECT src.id, src.d_code_id FROM v_espen_ng_lf_tas_2411_2_fts_yb_v2_3 src
                WHERE (SELECT count (*)  FROM v_espen_ng_lf_tas_2411_2_fts_yb_v2_3 inr WHERE src.d_code_id = inr.d_code_id ) > 1;

-- Check if there is solved orphaned participant then update
      IF EXISTS(
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_ng_lf_2411_tas1_duplicates_sk_kd m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_ng_lf_2411_tas1_duplicates_sk_kd m ON p.id = m.id_results            
            WHERE p.id IS NOT NULL
            ) and form = 'Résultat FTS';

      END IF;

       INSERT INTO metabase_ng_lf_2411_tas1_duplicates_sk_kd(id_results, barcode_results, form)
