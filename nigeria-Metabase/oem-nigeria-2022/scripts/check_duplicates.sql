...
*/
CREATE OR REPLACE FUNCTION update_ng_oncho_oem_duplicates_table_from_participant()
 RETURNS void
-- CREATE OR REPLACE PROCEDURE update_ng_oncho_oem_orphaned_table_from_participant()
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_oncho_oem_duplicates_of_participants p
       RIGHT JOIN metabase_ng_oncho_oem_duplicates_202204 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_ng_oncho_oem_duplicates_202204
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_oncho_oem_duplicates_of_participants p
            LEFT JOIN metabase_ng_oncho_oem_duplicates_202204 m ON p.id = m.id_participant
