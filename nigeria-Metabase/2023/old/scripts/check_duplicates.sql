...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_participants p
       RIGHT JOIN metabase_lf_pretas_duplicates_202102 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_pretas_duplicates_202102
          SET status = 'Solved'
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_duplicates_of_participants p
            LEFT JOIN metabase_lf_pretas_duplicates_202102 m ON p.id = m.id_participant
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_duplicates_of_fts_results p
       RIGHT JOIN metabase_lf_pretas_duplicates_202102 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_lf_pretas_duplicates_202102 m ON p.id = m.id_results            
            WHERE p.id IS NOT NULL
