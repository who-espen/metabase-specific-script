...
/*
 * Variable to rename metabase_lf_tas2_orphaned_202105, v_espen_bj_lf_tas2_5_partcipants_202104_v2,
...
*/
CREATE TABLE IF NOT EXISTS metabase_lf_tas2_orphaned_202105(
...
*/
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_participant_id_barcode_202105
    ON metabase_lf_tas2_orphaned_202105(id_participant, barcode_participant);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_results_id_barcode_202105
    ON metabase_lf_tas2_orphaned_202105(id_results, barcode_results);

  ALTER TABLE metabase_lf_tas2_orphaned_202105
 ...
 */
INSERT INTO metabase_lf_tas2_orphaned_202105(id_participant, recorder_id, barcode_participant, orphaned_type)
    ...
    FROM (
      SELECT
        ...
        FROM v_espen_bj_lf_tas2_5_partcipants_202104_v2 p
         LEFT JOIN v_espen_bj_lf_tas2_6_fts_result_202104_v2 d on p.p_code_id = d.d_code_id
          WHERE d.id isnull
          ) p
 ...
 */
CREATE OR REPLACE FUNCTION public.update_lf_tas2_orphaned_table_from_participant()
 RETURNS void
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_lf_orphaned_of_participants p
       RIGHT JOIN metabase_lf_tas2_orphaned_202105 m on p.id = m.id_participant
       WHERE p.id ISNULL
          ) THEN

          UPDATE metabase_lf_tas2_orphaned_202105
          SET status = 'Solved'          
          where id_participant NOT IN (
            SELECT p.id
            FROM v_lf_orphaned_of_participants p
            LEFT JOIN metabase_lf_tas2_orphaned_202105 m ON p.id = m.id_participant
...
-- Insert the new participant without diagnostic results to the orphaned table
      INSERT INTO metabase_lf_tas2_orphaned_202105(id_participant, recorder_id, barcode_participant, orphaned_type, survey_designation)
 ...
 */
INSERT INTO metabase_lf_tas2_orphaned_202105(id_results, recorder_id, barcode_results, orphaned_type)
  SELECT id, d_recorder_id, d_code_id, 'FTS results without participant'
    FROM (
      SELECT
        d.id, d_recorder_id, d_code_id
        FROM v_espen_bj_lf_tas2_5_partcipants_202104_v2 p
         RIGHT JOIN v_espen_bj_lf_tas2_6_fts_result_202104_v2 d on p.p_code_id = d.d_code_id
          WHERE p.id isnull
          ) p
...
-- Check if there is solved orphaned participant then update
      ...
      SELECT * FROM v_orphaned_of_diag_results p
       RIGHT JOIN public.metabase_lf_tas2_orphaned_202105 m on p.id = m.id_results
       WHERE p.id ISNULL
          ) THEN
          ...
          where id_results NOT IN (
            ...
            LEFT JOIN metabase_lf_tas2_orphaned_202105 m ON p.id = m.id_results            
             WHERE p.id IS NOT NULL
...
-- Insert the new participant without diagnostic results to the orphaned table
    INSERT INTO metabase_lf_tas2_orphaned_202105(id_results, recorder_id, barcode_results, orphaned_type, survey_designation)
