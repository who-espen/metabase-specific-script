 ...
 * Variable to rename v_espen_bj_oncho_stop_2_202306_questions_v3, <%v_ab_cde_fgh_3_rdt_ov16%>, <%metabase_oncho_oem_orphaned_202004%>
 */

SELECT
    p.p_cluster_id "Clustrer ID",
    NULL "Cluster Name",
    p_barecode_id "Barcode ID",
...
FROM
    ...
    LEFT JOIN < % v_ab_cde_fgh_3_rdt_ov16 % > d on p.p_barecode_id = d.d_barecode_id
...
SELECT
    ...
    status "Status"
FROM
    < % v_ab_cde_fgh_3_rdt_ov16 % > d
    LEFT JOIN v_espen_bj_oncho_stop_2_202306_questions_v3 p on p.p_barecode_id = d.d_barecode_id
