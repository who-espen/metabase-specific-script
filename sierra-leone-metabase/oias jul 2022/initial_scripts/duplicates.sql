...
/*
 * Variable to rename metabase_sr_oncho_oem_duplicates_202212, identify_participant_duplicate_pretas_202212, v_espen_sr_oncho_oem_2_participant_202211_v1_3,
 * v_espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2, metabase_sr_oncho_oem_result_duplicates_202212_trigger, metabase_sr_oncho_oem_duplicates_202212_trigger,
...
*/
  ...
  -- CREATE UNIQUE INDEX IF NOT EXISTS idx_duplicates_results_id_barcode_ng_oncho_oias_202212
  --   ON metabase_sr_oncho_oem_duplicates_202212(id_results, barcode_results);
      ...
      IF EXISTS(
        SELECT src.id, src.p_code_id FROM v_espen_sr_oncho_oem_2_participant_202211_v1_3 src
          WHERE src.p_code_id = NEW.p_code_id
            AND (SELECT count (*)  FROM v_espen_sr_oncho_oem_2_participant_202211_v1_3 inr WHERE src.p_code_id = inr.p_code_id ) > 1
            ) THEN
        ...
        INSERT INTO metabase_sr_oncho_oem_duplicates_202212(id_participant, barcode_participant, form)
          SELECT id, p_code_id, 'Participant'
            FROM (SELECT src.id, src.p_code_id FROM v_espen_sr_oncho_oem_2_participant_202211_v1_3 src
...
-- CREATE TRIGGER metabase_sr_oncho_oem_duplicates_202212_trigger AFTER INSERT OR UPDATE OR DELETE ON espen_ng_ng_oncho_oias_2_participant_202212_v2
...
AS $$
   BEGIN
      ...
      IF EXISTS(
        SELECT src.id, d_code_id FROM v_espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2 src
          WHERE d_code_id = NEW.d_code_id
            AND (SELECT count (*)  FROM v_espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2 inr WHERE src.d_code_id = inr.d_code_id ) > 1
            ) THEN

        INSERT INTO metabase_sr_oncho_oem_duplicates_202212(id_results, barcode_results, form)
          SELECT id, d_code_id, 'Diagnostic'
            FROM (SELECT src.id, d_code_id FROM v_espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2 src
              WHERE d_code_id = NEW.d_code_id) p
          ON CONFLICT ON CONSTRAINT unique_idx_duplicates_results_id_barcode_ng_oncho_oias_202212 DO NOTHING;
...
CREATE TRIGGER metabase_sr_oncho_oem_result_duplicates_202212_trigger AFTER INSERT OR UPDATE OR DELETE ON espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2
...
*/
 ...
 SELECT id, d_code_id, 'Diagnostic'
            FROM (
              SELECT src.id, src.d_code_id FROM v_espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2 src
                WHERE (SELECT count (*)  FROM v_espen_ng_ng_oncho_oias_3_resultat_fts_202212_v2 inr WHERE src.d_code_id = inr.d_code_id ) > 1
            ) p
