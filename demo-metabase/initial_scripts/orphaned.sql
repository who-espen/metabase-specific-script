...
/*
 * Variable to rename metabase_ng_lf_pretas_orphaned_202212, v_espen_ng_lf_pretas_2_participant_202102_v2_1,
...
CREATE TABLE
    IF NOT EXISTS metabase_ng_lf_pretas_orphaned_202212(
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_participant_id_barcode_pretas_202212 ON metabase_ng_lf_pretas_orphaned_202212(
    id_participant,
    barcode_participant
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_orphaned_results_id_barcode_pretas_202212 ON metabase_ng_lf_pretas_orphaned_202212(id_results, barcode_results);

ALTER TABLE
    metabase_ng_lf_pretas_orphaned_202212
ADD
    CONSTRAINT unique_idx_orphaned_participant_id_barcode_pretas_202212 UNIQUE USING INDEX idx_orphaned_participant_id_barcode_pretas_202212;

ALTER TABLE
    metabase_ng_lf_pretas_orphaned_202212
ADD
    CONSTRAINT unique_idx_orphaned_results_id_barcode_pretas_202212 UNIQUE USING INDEX idx_orphaned_results_id_barcode_pretas_202212;
...
FROM (
        ...
        FROM
            ...
            LEFT JOIN v_espen_ng_lf_pretas_3_resultat_fts_202102_v2_1 d on p.p_code_id = d.d_code_id
        WHERE
            d.id isnull
    ) p ON CONFLICT ON CONSTRAINT unique_idx_orphaned_participant_id_barcode_pretas_202212
DO NOTHING;
...
LANGUAGE PLPGSQL AS 
	...
	OR
	REPLACE
	    TEMPORARY VIEW v_ng_lf_orphaned_of_participants_202212 AS
	SELECT
	    p.id,
	    p.p_recorder_id,
	    p.p_code_id
	...
	IF EXISTS(
	    SELECT *
	    FROM
	        v_ng_lf_orphaned_of_participants_202212 p
...
FROM (
        ...
        FROM
            v_espen_ng_lf_pretas_2_participant_202102_v2_1 p
            RIGHT JOIN v_espen_ng_lf_pretas_3_resultat_fts_202102_v2_1 d on p.p_code_id = d.d_code_id
        WHERE
            p.id isnull
    ) p ON CONFLICT ON CONSTRAINT unique_idx_orphaned_results_id_barcode_pretas_202212
DO NOTHING;
