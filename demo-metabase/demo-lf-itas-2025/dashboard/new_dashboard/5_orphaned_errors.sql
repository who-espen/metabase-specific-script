 ...
 p
    where (select count(*) from v_espen_demo_lf_tas_9999_1_site_v3 as 
 k where p.p_code_id = k.p_code_id) > 1
    ...
    FROM public.v_espen_demo_lf_itas_9999_3_fts d
    where (select count(*) from v_espen_demo_lf_itas_9999_3_fts r where d.d_code_id = r.d_code_id) > 1