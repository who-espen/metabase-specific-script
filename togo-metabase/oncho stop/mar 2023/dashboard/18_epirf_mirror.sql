 ...
 * Variable to rename <%Epirf_Survey_Type%>, v_espen_tg_oncho_stop_202304_3_elisa, v_espen_tg_oncho_stop_202304_2_participants,
...
LEFT JOIN v_espen_tg_oncho_stop_202304_1_site c on d.d_site_id = c.c_site_id
RIGHT JOIN v_espen_tg_oncho_stop_202304_2_participants p on p.p_code_id = d.d_code_id

GROUP BY c_eu_name, c_commune, c_site, "Mois", "Années", c_gps_lat, c_gps_lng

order by c_eu_name, c_commune, c_site
 ...
 SELECT
  ...
  'école' "Site enquete",
  'grappes' "Type d'enquête", -- TODO: Update the survey type
  count(distinct c.c_site_id)*50 "Taille Population cible",-- TODO: Update the sample size
  count(p.id) "Examiné",
  COUNT(CASE WHEN d_lab_elisa = 'Positive' THEN 1 ELSE NULL END) "Number de Positif",
  ROUND(COUNT(CASE WHEN d_lab_elisa = 'Positive' THEN 1 ELSE NULL END) * 100.0 / count(p.id), 2) "% positif",
  null "Décision", --TODO: Update the decision
