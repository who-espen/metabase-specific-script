...
LANGUAGE PLPGSQL AS $$ 
	...
	SELECT
	    ...
	    src.p_barcodeid
	FROM
	    v_espen_tg_sch_sth_impact_202309_2_child_v2 src
	WHERE (
	        SELECT count (*)
	        FROM
	            v_espen_tg_sch_sth_impact_202309_2_child_v2 inr
	        WHERE
...
OR
REPLACE
    TEMPORARY VIEW v_espen_tg_sch_sth_impact_202309_3_kato_katz_v2 AS
SELECT
    src.id,
    src.k_barcodeid
FROM
    v_espen_tg_sch_sth_impact_202309_3_kato_katz_v2 src
WHERE (
        SELECT count (*)
        FROM
            v_espen_tg_sch_sth_impact_202309_3_kato_katz_v2 inr
        WHERE
...
IF EXISTS(
    SELECT *
    FROM
        v_espen_tg_sch_sth_impact_202309_3_kato_katz_v2 p
...
FROM
    metabase_tg_202309_sch_sth_impact_duplicates m
    LEFT JOIN v_espen_tg_sch_sth_impact_202309_3_kato_katz_v2 p on p.id = m.id_results_kk:: int
WHERE
    p.id ISNULL
...
SELECT
    ...
    src.u_barcodeid
FROM
    v_espen_tg_sch_sth_impact_202309_4_urine_filtration_v2_1 src
WHERE (
        SELECT count (*)
        FROM
            v_espen_tg_sch_sth_impact_202309_4_urine_filtration_v2_1 inr
        WHERE
